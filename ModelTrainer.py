...
from ClassifierModels import Resnet18, DenseNet121
...
class ModelTrainer:
    ...
    def __init__(self, architecture_type, num_of_input_channels, is_backbone_trained, num_classes, device):
        ...
        elif self.architecture_type == 'DENSE-NET-121':
            self.model = DenseNet121(self.num_classes, is_backbone_trained).to(self.device)
    ...
    def test(self, path_img_dir, path_file_test, path_trained_model,
        ...
        with torch.no_grad():
            for batch_id, (input_img, target) in enumerate(data_loader_test):
                # if np.mod(batch_id, 10) == 0:
                #     print('.', end="", flush=True)
                print(batch_id + 1, '/', len(data_loader_test))
                ...
                if self.architecture_type == 'RES-NET-18' or self.architecture_type == 'AE-RES-NET-18' or self.architecture_type == 'DENSE-NET-121':
